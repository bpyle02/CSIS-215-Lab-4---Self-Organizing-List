...
template <typename E> class CountSOL : public SelfOrderedListADT<E>
{
	LList<E>* list;
	int comparisons;
...
public:
	...
	//This function reorders the nodes using the Count Heuristic
	//The Count Heuristic simply moves the record forward in the
	//list based on how many times it has been accessed. The more
	//accesses, the closer the record will be to the front
	void reorder(int pos)
	{
		...
		//The following code will execute only if the position is greater than 0
		if (pos != 0)
		{
			list->moveToPos(pos); //Moves to the node that is found in the find() function

			//The following code will execute only while the frequency of the next element
			//is greater than the frequency of the current element
			while (list->isFrequencyOutOfOrder())
			{
				list->moveToPos(pos); //Moves to the node that is found in the find() function
				...
				pos--; //Decrements the position variable
				list->moveToPos(pos); //Moves to the new position
